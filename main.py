...
# totosize -1 as we have already took the values out from the first page
for i in range(1, totosize + 1):
    Selector = Select(driver.find_element_by_css_selector("select.form-control.selectDrawList"))
    Selector.select_by_value(str((3400 + totosize) - i))
    drawno = driver.find_element_by_class_name("drawNumber")
    drawnoList.append(drawno.text.split(" ")[2])
    group1WShares = driver.find_element_by_xpath(
        "//*[@id='ctl00_ctl36_g_7daddb1d_8fe5_43c9_ba4a_6a00b22a7111_ctl00_divSingleResult']/div[1]/table[5]/tbody/tr[2]/td[3]")
    ...
    try:
        group1WLocation = driver.find_element_by_xpath(
    ...
    if group1WLocation.text == "Group 1 winning tickets sold at:":
        locationlen = len(driver.find_element_by_xpath(
            "//*[@id='ctl00_ctl36_g_7daddb1d_8fe5_43c9_ba4a_6a00b22a7111_ctl00_divSingleResult']/div[2]/ul[1]").text.splitlines())
        if locationlen > 1:
            for i in range(0, locationlen):
                locationdetails = driver.find_element_by_xpath(
                    "//*[@id='ctl00_ctl36_g_7daddb1d_8fe5_43c9_ba4a_6a00b22a7111_ctl00_divSingleResult']/div[2]/ul[1]").text.splitlines()[
                    i]
                finalLocDetails = finalLocDetails + locationdetails + "$"
            group1WLocList.append(finalLocDetails[:-1])
        else:
            group1WLocList.append(driver.find_element_by_xpath(
                "//*[@id='ctl00_ctl36_g_7daddb1d_8fe5_43c9_ba4a_6a00b22a7111_ctl00_divSingleResult']/div[2]/ul[1]").text)
    else:
        group1WLocList.append("Nil")
    group1DrawDate = driver.find_element_by_xpath(
        "//*[@id='ctl00_ctl36_g_7daddb1d_8fe5_43c9_ba4a_6a00b22a7111_ctl00_divSingleResult']/div[1]/table[1]/thead/tr/th[1]")
